...
setwd("C:/Users/noahh/OneDrive - Uppsala universitet/Erasmus Oslo/01_Project/04_Data/Pymol_RMSD/")

#Formatting the tables an reading in the csv
file_names<- list.files(,pattern="np.csv")
names_col <- c("rmsd_crystal_ABB","rmsd_crystal_AF2","rmsd_crystal_IgFold", "rmsd_ABB_AF2","rmsd_ABB_IgFold","rmsd_AF2_IgFold")

H_chain <- read.csv(file_names[1], head =F)
colnames(H_chain) <- names_col

L_chain <- read.csv(file_names[2])
colnames(L_chain) <- names_col

Paired_chain <- read.csv(file_names[3])
colnames(Paired_chain) <- names_col
...
colnames(df_k) <- c("RMSD", "classifier")
...
for (i in 1:6){
  rmsd = i
  classifier = 6 + i
  df_k[1:nrow(input),c(1,2)] <- input[,c(rmsd,classifier)]
...
plot1 <- ggplot(boxplot_X_chain, aes(x = classifier, y = RMSD))+
  ...
  stat_summary(aes(x=classifier,y = RMSD),
   ...
   geom_label(data = summ_RMSD_X, aes(x = classifier, y = -1.5, label = paste("Mean: ", round(mean, 3), "\nMedian: ",round(median, 3), "\nSD: ", round(sd, 3))), size = 2, label.size = 0, label.padding = unit(0.1, "lines")) + #Adds labels
...
```{r}
plot4<- RMSD_plot(boxplot_L_chain, "light", summ_RMSD_L, geom_violin)
plot4
...
geom_signif(aes(x=classifier, y = RMSD), comparisons = list(c("crystal_AF2", "crystal_IgFold"),
                                                            c("crystal_ABB", "crystal_IgFold"),
                                                            c("crystal_ABB", "crystal_AF2")),
                                                            map_signif_level = TRUE, y_position = c(25,26.5,25))
return(plot1)
}

plot5<- RMSD_plot(boxplot_Paired_chain, "paired", summ_RMSD_Paired, geom_boxplot)
plot5
...
```{r}
plot6<- RMSD_plot(boxplot_Paired_chain, "paired", summ_RMSD_Paired, geom_violin)
plot6
...
write.csv(boxplot_H_chain, "data_H_chain_Pymol.csv")
write.csv(boxplot_L_chain, "data_L_chain_Pymol.csv")
write.csv(boxplot_Paired_chain, "data_Paired_chain_Pymol.csv")

write.csv(summ_RMSD_H, "summ_RMSD_H_Pymol.csv")
write.csv(summ_RMSD_L, "summ_RMSD_L_Pymol.csv")
write.csv(summ_RMSD_Paired, "summ_RMSD_Paired_Pymol.csv")
